...
city.get("/", (req, res) => {
  ...
  if("error" in e){
    ...
    return res.render(path + '/pages/category.ejs', {logged:false,posts:db.get("posts").value()})
      }
...
Log("System",`${c.user.username} kategorilere  giriş yaptı`)
return res.render(path + '/pages/category.ejs', {client:aclient,logged:true,posts:db.get("posts").value()})
    
})} else {return res.render(path + '/pages/category.ejs', {logged:false,posts:db.get("posts").value()})}
    // giriş yapılmamış
